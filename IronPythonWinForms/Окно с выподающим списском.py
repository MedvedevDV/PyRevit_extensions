...
winSize = Size(600, 300)  # размеры winform (ширина(min = 300), высота).
...
class WinForm(Form):
    ...
    def add_txtbox(self,uiW, position =1, value="00_, 01_, 02_, 03_, Связь"): #- если нужно в рамку добавить тест описания
        # окно ввода данных
        text_box = TextBox()
    ...
    def add_combov_verd(self,uiW,position =1,name = "имя", value = ["Пример 1", 'Пример 2']):
        ...
        combov_verd.Location = Point(btnSpacing, (st_height + htSpacing * position))
        return combov_verd

    def okButton_pressed(self, sender, args):
        numErrorProvider = System.Windows.Forms.ErrorProvider()
        try:
            for lst_combov_verd in self.lst_combov_verds:
                out.append(lst_combov_verd.Text)
    ...
    def __init__(self):
        ...
        #-----Добавляем выподающий список-----#
        self.combov_verd1 = self.add_combov_verd(uiWidth * 2,1, "Файлы", ["ф1",'ф2','ф3','ф4','ф5',])
        self.Controls.Add(self.combov_verd1)
        self.lst_combov_verds = [self.combov_verd1]

        # ----- добавить ввод данных с клавиатуры -----#
        # Добавляем текстовый логотип ввода
        self.Controls.Add(self.add_txtbox_label(2, "Закрыть рабочие наборы:"))
        self.textBOX = self.add_txtbox(uiWidth, 2)
        self.lst_txtBoxses = [self.textBOX]
