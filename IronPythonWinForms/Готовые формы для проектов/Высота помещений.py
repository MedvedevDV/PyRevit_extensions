...
winSize = Size(400.0, 600)  # размеры winform (ширина(min = 300), высота).
...
def logo(uiW, uiH):
    ...
    ratio = float(logo.Height) / float(logo.Width)  # должно быть значение с плавающей точкой, так как int округляется до ближайшего целого числаr
    logo.Size = Size(200, 80 * ratio)  # fixed
    ...
    logo.Anchor = (
                AnchorStyles.Bottom | AnchorStyles.Left)  # Привязка для созранения полодения при масштабировании окна
    return logo
...
class WinForm(Form):
    ...
    def __init__(self):
        ...
        #----- добавить флажек() -----#
        self.checkbox1 = self.add_CHECKBOX(uiWidth, 6, "Антрисоль", "Есть антрисольные помещения?")
        self.chBox_checks = [self.checkbox1]
        ...
        self.lst_txtBoxses = [self.txtBox_podpolie, self.txtBox_level01,self.txtBox_level02, self.txtBox_level03, self.txtBox_level04, self.txtBox_ant]
