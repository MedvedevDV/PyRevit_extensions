...
if not_view_section_conditions:
    ...
    if not fe_symbol:
        family_path = "\\\\fs\\public\\Холдинг\\ПоревитД\\ТИМ\\01_Библиотека\\01_Рабочие задачи\\ALL_Аннотации общие\\PS_ФлажокПирогаКонструкции.rfa"
        try:
    ...
    while True:
        try: 
            ...
            point1 = selection.PickPoint(ObjectSnapTypes.Intersections,'1) Выберите точку начала выноски:')

            #with forms.WarningBar(title='2) Выберите точку рсположения отметки:'):
            pt2 = selection.PickPoint(ObjectSnapTypes.Intersections,'2) Выберите точку рсположения отметки:')
            ...
            with Transaction(doc, 'Создать флажок') as t:
                ...
                ot = new_instance.LookupParameter("Положение выноски").Set(loc_leader)
                #print(ot)
