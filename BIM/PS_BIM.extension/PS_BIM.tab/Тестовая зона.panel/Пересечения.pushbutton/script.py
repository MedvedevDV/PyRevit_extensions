...
folder_path = r'\\fs\bim\Projects\00.BIM_Export\Проверки на пересечения'

# Проверяем, существует ли папка
if os.path.exists(folder_path) and os.path.isdir(folder_path):
    try:
        # Формируем команду для копирования пути в буфер обмена без кавычек
        clip_command = 'echo ' + folder_path + ' | clip'
        
        # Выполняем команду
        subprocess.Popen(clip_command, shell=True)
    except Exception:
        pass  # Игнорируем возможные ошибки копирования
else:
    pass  # Игнорируем ошибку, если папка не существует



# Параметры
DLL_NAME = 'FindIntersection.dll'  # Имя вашего DLL файла
COMMAND_NAME = 'FindIntersection.FindIntersectionCommand'  # Полное имя класса с пространством имен
