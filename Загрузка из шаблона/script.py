...
def take_pasword(ws_doc, info):
    ...
    if REQUEST_PASWARD:
        ...
        if paswor != '5720':
            forms.alert(msg='Пароль не правильный',warn_icon='error')
            ws_doc.Close(False)
            sys.exit()
...
for id in uidoc.Selection.GetElementIds():
    ...
    if element.Category.Name in TYPES_TO_COPY[1:]:
        forms.alert('Через предварительный выбор можно обновить только загружаемые семейства!\n\nВыберите семейство(а) и попробуйте еще раз.',warn_icon='error')
        sys.exit()
    else:
        try:
...
if form.values:
    ...
    doc_filtered_families = [
            fam for name, fam in doc_name_family_dict.items() 
            if name and ("PS_" in name or "ADSK_" in name)
        ]
    
    doc_famly_values = {}
    for fam in doc_filtered_families:
        try:
            symbol_ids = list(fam.GetFamilySymbolIds())[0]
            if symbol_ids:
                param = doc.GetElement(symbol_ids).LookupParameter("ADSK_Версия семейства")
                doc_famly_values[fam.Name] = param
        except Exception as e:
            # forms.alert(msg="Найдено семейство без типов: {}.\nУдалдите это семейсто из проетка.".format(fam.Name),warn_icon='error')
            pass
            # sys.exit()
